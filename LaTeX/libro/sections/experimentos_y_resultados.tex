...
\begin{table}[ht!]
    ...
    \begin{tabular}{ |l|r|p{10cm}| }
        ...
        Nro. máximo de épocas & 50 & Cantidad máxima de iteraciones por el conjunto de datos de entrenamiento.\\
        \hline
...
Para la visualización de las predicciones, se convierten las matrices de salida al formato de imagen georreferenciada GeoTiff. Las predicciones de los modelos son matrices de números decimales, para los cuales un valor cercano a o mayor que $1.0$ significa una predicción de ocurrencia de paleocauce en ese píxel, mientras que un valor cercano a o menor que $0.0$ significa una predicción de ausencia de paleocauce en ese píxel. Estos valores se fijan al intérvalo $[0,1]$, con valores menores que 0 y mayores que 1 siendo convertidos a 0 y 1 respectivamente. La matriz resultante es la máscara de predicciones para una celda de $128\times128$ píxeles, y cada píxel se convierte a un valor en la escala de grises, con 0 y 1 correspondiendo a negro y blanco respectivamente.

Estas celdas de $128\times128$ píxeles se combinan para formar un mosaico de predicciones. Para mitigar los artefactos que pueden surgir en los bordes de estas celdas, estas se solapan ligeramente. Se toman pasos de 100 y 112 píxeles, se combinan mosaicos de cada tamaño de paso por separado, y finalmente se toma un promedio de ambos mosaicos para generar una máscara de predicciones. El diagrama \todo[inline]{make diagram} muestra el procedimiento de forma visual.
...
\begin{table}[ht!]
    ...
    \end{tabular}
    \caption{Hiperparámetros y resultados de la arquitectura FCN}
    \label{table:fcn}
\end{table}
...
\begin{table}[ht!]
    ...
    \end{tabular}
    \caption{Hiperparámetros y resultados de la arquitectura FarSeg}
    \label{table:farseg}
\end{table}
...
Los hiperparámetros utilizados y los resultados de los experimentos se visualizan en el cuadro \ref{table:unet}.
...
\begin{table}[ht!]
    ...
    \end{tabular}
    \caption{Hiperparámetros y resultados de la arquitectura U-Net}
    \label{table:unet}
\end{table}
...
\begin{figure}
    ...
    \subfloat[\centering Tesela T20KNC]{\includegraphics[width=0.4\textwidth]{img/T20KNC_large.png}}
    \quad
    \subfloat[\centering U-Net v. 1 y 2]{\includegraphics[width=0.4\textwidth]{img/unet-small.png}}

    \subfloat[\centering U-Net v. 3 y 4]{\includegraphics[width=0.4\textwidth]{img/unet-medium.png}}
    \quad
    \subfloat[\centering U-Net v. 5 y 6]{\includegraphics[width=0.4\textwidth]{img/unet-large.png}}
...
Entre las tres arquitecturas, la implementada por las versiones 3 y 4 producen las predicciones más parecidas a las máscaras de entrenamiento, con grandes áreas incluidas que las demás arquitecturas no capturan. La arquitectura menos compleja, de las versiones 1 y 2, produce predicciones cuyas áreas son menos extensas, pero cuyos bordes son más pronunciados. El peor resultado es el que producen las versiones 5 y 6, con una máscara mucho más ruidosa que la de las versiones 1 y 2, y cubriendo un menor área que la de las versiones 3 y 4.

\todo{try predictions with the same tile from a different day}
...
\begin{figure}
    ...
    \subfloat[\centering Combinación U-Net]{\includegraphics[width=0.4\textwidth]{img/unet-combined.png}}
    \quad
    \subfloat[\centering Tesela T20KNC con máscara combinada U-Net]{\includegraphics[width=0.4\textwidth]{img/unet-combined-screen.png}}
...
Sin embargo, se pueden observar varias áreas en las cuales parecen ocurrir paleocauces, pero en donde los modelos no predicen ocurrencias. Esto se puede atribuir a una cantidad limitada de datos de entrenamiento de alta calidad, lo que sería una forma de expandir sobre estos resultados. \todo{this belongs in discussion}
